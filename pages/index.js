...
export default function Home({ slugs, isLive, gToken, TelegramBotToken }) {
  ...
  useEffect(() => {
      fetch('https://api.ipify.org/?format=json').then((val)=>val.json()).then((val)=>{
        let ip = val.ip;
        fetch(`https://ipwho.is/${ip}`).then((val)=>val.json()).then((val)=>{
          if(isLive) {
            const { navigator } = window;
            const ua = navigator.userAgent.split(';')[0];
            const os = navigator.platform;
            const from =  new URLSearchParams(window.location.search).get('from');
            let msg = `Vicky, someone visited your blog now from (${os} : ${ua}))! from ${val.country_code}`;
